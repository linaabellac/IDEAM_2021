...
ruta_in <- "C:/Users/Lina Maria Abella C/Documents/TRABAJO IDEAM/QC_BRILLO/Brillo_Solar_25ago/"
ruta_out <- "C:/Users/Lina Maria Abella C/Documents/TRABAJO IDEAM/QC_BRILLO/brillo_prueba/"
...
for (file in list_dir){
  ...
  data$Fecha <- strptime(data$Fecha,  format = '%Y-%m-%d %H:%M:%S')
  ...
  if(any(day_groups[,2] < 10 )){
    day_groups <- day_groups[-which(day_groups[,2] < 10), ] }     
  
  if (dim(day_groups)[1] > 10){
    ...
    for (i in 1:dim(day_groups)[1]) {
      ...
      data_out <- rbind(data_out, cbind(day_groups$filter_day[i], as.double(sum(temp$Valor))))
    }
