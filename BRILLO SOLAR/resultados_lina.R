...
Datos <- file.path("regiones_crudas", "TEST_UNIDOS", "REGION1")
outPath <- file.path("regiones_crudas", "TEST_UNIDOS",'RESULTADOS_LINA', 'REGION1')
...
for (file in list_dir){ 
  ...
  colnames(serie_final)<- c('Fecha', 'Valor','Valor_mod','Estado')
  
  write.table(resultados_lina, file.path(outPath, paste0('BSHG_TT_D@', code_station, '.data')), sep="|", row.names = FALSE, col.names = TRUE)
