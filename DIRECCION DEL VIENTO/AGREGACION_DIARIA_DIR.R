...
ruta_in <- "C:/Users/Lina Maria Abella C/Documents/TRABAJO IDEAM/VELOCIDAD Y DIRECCION VIENTO/DIRECCION_AGREGADAS/"
ruta_out <- "C:/Users/Lina Maria Abella C/Documents/TRABAJO IDEAM/VELOCIDAD Y DIRECCION VIENTO/DIRECCION_DIARIAS/"
...
for (file in list_dir){
  ...
  if (dim(day_groups)[1] > 16){
    ...
    write.table(data_final, file.path(ruta_out, paste0('DV_10_VECT_MEDIA_D@', code_station, '.data')), sep="|", row.names = FALSE, col.names = TRUE)
    tx  <- readLines(file.path(ruta_out, paste0('DV_10_VECT_MEDIA_D@', code_station, '.data')))
    tx2  <- gsub(pattern = '"', replace = '', x = tx)
    writeLines(tx2, con=file.path(ruta_out, paste0('DV_10_VECT_MEDIA_D@', code_station, '.data')))
