...
ruta_in <- "C:/Users/Lina Maria Abella C/Documents/TRABAJO IDEAM/VELOCIDAD Y DIRECCION VIENTO/CONTROLES/VV_10_MEDIA_H/"
ruta_out <- "C:/Users/Lina Maria Abella C/Documents/TRABAJO IDEAM/VELOCIDAD Y DIRECCION VIENTO/CONTROLES/SALIDA_AGREG/"
...
for (file in list_dir){
  ...
  if (dim(day_groups)[1] > 16){
    ...
    for (i in 1:dim(day_groups)[1]) {
      ...
      data_out <- rbind(data_out, cbind(day_groups$filter_day[i], as.double(mean(temp$Valor, na.rm = T)),sum(temp$Estado)))
    }
    
    data_out <- data_out[-1,]
    colnames(data_out) <- c("Fecha", "Valor", 'Estado')
    ...
    write.table(data_out, file.path(ruta_out, paste0('VV_10_MEDIA_D@', code_station, '.data')), sep="|", row.names = FALSE, col.names = TRUE)
    tx  <- readLines(file.path(ruta_out, paste0('VV_10_MEDIA_D@', code_station, '.data')))
    tx2  <- gsub(pattern = '"', replace = '', x = tx)
    writeLines(tx2, con=file.path(ruta_out, paste0('VV_10_MEDIA_D@', code_station, '.data')))
