...
ruta_in <- "C:/Users/Lina Maria Abella C/Documents/TRABAJO IDEAM/HUMEDAD RELATIVA/HR_Automáticas_SEP01_2021/HR2_AUT_60/"
ruta_out <- "C:/Users/Lina Maria Abella C/Documents/TRABAJO IDEAM/HUMEDAD RELATIVA/HR_Automáticas_SEP01_2021/Agregadas_1sep/"
...
for (file in list_dir){
  ...
  if (dim(day_groups)[1] > 16){
    ...
    write.table(data_out, file.path(ruta_out, paste0('HRA2_MEDIA_D@', code_station, '.data')), sep="|", row.names = FALSE, col.names = TRUE)
    tx  <- readLines(file.path(ruta_out, paste0('HRA2_MEDIA_D@', code_station, '.data')))
    tx2  <- gsub(pattern = '"', replace = '', x = tx)
    writeLines(tx2, con=file.path(ruta_out, paste0('HRA2_MEDIA_D@', code_station, '.data')))
