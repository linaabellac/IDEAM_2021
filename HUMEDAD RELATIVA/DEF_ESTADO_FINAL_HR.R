...
Datos <- file.path("SALIDAS_24SEP", "REGION11")
outPath <- file.path('SALIDAS_24SEP','SALIDAS_ALEX', 'REGION11')
...
for (etiqueta in variables) {
  ...
  for (file in list_dir){ 
    ...
    write.table(serie_final, file.path(outPath, paste0(etiqueta,'@',code_station, '.data')), sep="|", row.names = FALSE, col.names = TRUE)
