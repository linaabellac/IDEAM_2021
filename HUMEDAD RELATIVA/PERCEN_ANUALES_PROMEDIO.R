...
df<-data.frame(t(round(prom,2)))
colnames(df)<-c('año', '1%', '25%', '75%', '99%')


write.table(df, file.path(ruta_in, paste0('REGION12_PERCENTIL_PROM.data')), sep="|", row.names = FALSE, col.names = TRUE)
tx  <- readLines(file.path(ruta_in, paste0('REGION12_PERCENTIL_PROM.data')))
tx2  <- gsub(pattern = '"', replace = '', x = tx)
writeLines(tx2, con=file.path(ruta_in, paste0('REGION12_PERCENTIL_PROM.data')))
