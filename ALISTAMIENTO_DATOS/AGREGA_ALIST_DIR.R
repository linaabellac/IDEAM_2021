...
ruta_in <- "C:/Users/Lina Maria Abella C/Documents/TRABAJO IDEAM/VELOCIDAD Y DIRECCION VIENTO/DIRECCION_CONTROLES/SALIDAS_AGREGAR_ALIST/DVAG_CON/"
ruta_out <- "C:/Users/Lina Maria Abella C/Documents/TRABAJO IDEAM/VELOCIDAD Y DIRECCION VIENTO/DIRECCION_CONTROLES/SALIDAS_PARA_ESTADO/"
...
for (file in list_dir){
  ...
  if (dim(day_groups)[1] > 16){
    ...
    for (i in 1:dim(day_groups)[1]) {
      ...
      data_out <- rbind(data_out, cbind(t(day_groups$filter_day[i]), as.double(mean(temp$seno)),as.double(mean(temp$coseno)),sum(temp$Estado)))
    }
