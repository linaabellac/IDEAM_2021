...
Datos <- file.path('SALIDAS_AGRE_DIARIA')
outPath <- file.path('SALIDAS_FIN_ALIST')
...
for (etiqueta in variables) {
  ...
  for (file in list_dir){ 
    ...
    serie_nulos<-data.frame(data$Fecha, data$Valor, estado_final);colnames(serie_nulos)<-c('Fecha', 'Valor', 'Estado')
