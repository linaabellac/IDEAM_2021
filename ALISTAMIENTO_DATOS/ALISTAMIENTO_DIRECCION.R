...
Datos <- file.path('Output_DV_20211114', 'Complementacion')
datos_variable<-file.path('SALIDAS_X_ALIST_DIR')
outPath <- file.path('DIRECCION_VIENTO')
...
for (etiqueta in variables) {
  ...
  list_dir_var<-list.files(file.path(datos_variable,etiqueta),full.names = F)
  ...
  for (file in list_dir){ 
    ...
    if (any(list_dir_var==file)) {
      ...
      data_v<- data.frame(read.table(file.path(datos_variable, etiqueta, file), sep = '|', header = T))}
    
    temp<-filter(data, data$status %in% c('1HOM0','1COM0'))
    
    colnames(data_v)<-c('event_time','Valor','event_value', 'status')
    data_v$station<-code_station
    data_v$sensor<-etiqueta
    ...
    data_out$event_value<-data_out$event_value *180/pi
    data_out$event_value<-round(data_out$event_value,4)
    ...
    write.table(data_out, file.path(outPath, paste0(code_station,'-',etiqueta, '.csv')), row.names = F, col.names = F, sep = ",")
    tx  <- readLines(file.path(outPath, paste0(code_station,'-',etiqueta, '.csv')))
    tx2  <- gsub(pattern = '"', replace = '', x = tx)
    writeLines(tx2, con=file.path(outPath, paste0(code_station,'-',etiqueta, '.csv')))
  }
}
