...
Datos <- file.path('CONTEO_ATIP_DIARIO_VEL')
...
for (etiqueta in variables) {
  ...
  for (file in list_dir){ 
    ...
    colnames(df)<-c('Codigo','variable','%atip_final')
...
write.table(df, file.path(outPath, paste0('REPORTE_VELO.data')), sep="|", row.names = FALSE, col.names = TRUE)
tx  <- readLines(file.path(outPath, paste0('REPORTE_VEL.data')))
tx2  <- gsub(pattern = '"', replace = '', x = tx)
writeLines(tx2, con=file.path(outPath, paste0('REPORTE_VELO.data')))
