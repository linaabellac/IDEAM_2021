...
ruta_in <- "C:/Users/Lina Maria Abella C/Documents/TRABAJO IDEAM/VELOCIDAD Y DIRECCION VIENTO/CONTROLES/SALIDAS_AGRE_ALIST/"
ruta_out <- "C:/Users/Lina Maria Abella C/Documents/TRABAJO IDEAM/VELOCIDAD Y DIRECCION VIENTO/CONTROLES/SALIDAS_AGRE_DIARIA/"
...
for (file in list_dir){
  ...
  if (dim(day_groups)[1] > 16){
    ...
    for (i in 1:dim(day_groups)[1]) {
      ...
      data_out <- rbind(data_out, cbind(day_groups$filter_day[i], as.double(mean(temp$Valor)),sum(temp$Estado)))
    }
